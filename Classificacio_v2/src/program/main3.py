...
if os.path.isfile('D:\\ICAR\\Results\\Classificacio3\\objs.pickle'):
    with open('D:\\ICAR\\Results\\Classificacio3\\objs.pickle','rb') as f:
    ...
    with open('D:\\ICAR\\Results\\Classificacio3\\objs.pickle', 'wb') as f:
...
improc=0.0
contok=0.0
fileout=open('D:\\ICAR\\Results\\Classificacio3\\confmatORB.dat','w')
...
f=open('D:\\ICAR\\Results\\Classificacio3\\confmatORB.dat')
data=f.readlines()
f.close()
...
np.savetxt("D:\\ICAR\\Results\\Classificacio3\\matrix.txt", np.asarray(cm2,np.uint32), fmt='%d')

# write the list of test labels
f=open("D:\\ICAR\\Results\\Classificacio3\\test.txt",'w')
for y in y_test :
    f.write(y+"\n");
f.close();
    
# write the list of pred labels
f=open("D:\\ICAR\\Results\\Classificacio3\\pred.txt",'w')
for y in y_pred :
    f.write(y+"\n");
f.close();
